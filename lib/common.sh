...
function copy_docker_build_files() { 
  ...
  local docker_compose_template="$DIB_APP_COMPOSE_DOCKER_COMPOSE_TEMPLATE_FILE"
...
function update_other_variables_if_changed() {
  ...
  if [[ -n "$USER_DIB_APP_PORTS" && "$DIB_APP_PORTS" != "$USER_DIB_APP_PORTS" ]]
  then
    export DIB_APP_PORTS=$USER_DIB_APP_PORTS
    export DIB_APP_CONTAINERS_RESTART=1
  fi

  if [[ -n "$USER_DIB_APP_ENV_FILES" && "$DIB_APP_ENV_FILES" != "$USER_DIB_APP_ENV_FILES" ]]
  then
    export DIB_APP_ENV_FILES=$USER_DIB_APP_ENV_FILES
    export DIB_APP_CONTAINERS_RESTART=1
  fi
}
